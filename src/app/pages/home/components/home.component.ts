...
export class HomeComponent implements OnInit {
  ...
  private storeSubscription(){
    ...
    this.postStore
      ...
      .subscribe((response) => {
        if(response?.state){
          const index = this.listColumns.findIndex((element) => response.items.map((item) => item.map((task) => task.id_column == element.id)));
          this.listColumns[index].task_list = response.items[0];
        }
      })
  }
